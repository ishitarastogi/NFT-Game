...
contract DungeonsAndDragonsCharacter is ERC721URIStorage, VRFConsumerBase, Ownable {
 ...
 //Each oracle job has a unique Key Hash, which is used to identify tasks that it should perform. 
 //The contract will store the Key Hash that identifies Chainlink VRF, and the fee amount, 
 //to use in the request.
    ...
    function requestNewRandomCharacter(
        ...
        require(
            LINK.balanceOf(address(this)) >= fee,  //check if contracts has enough link to pay for the fees
            "Not enough LINK - fill contract with faucet"
        );
        bytes32 requestId = requestRandomness(keyHash, fee); // this function  Make a request to the VRF coordinator.
// vrf coordinator make sure everything happens correctly
//this function then make call to fulfill randomness
